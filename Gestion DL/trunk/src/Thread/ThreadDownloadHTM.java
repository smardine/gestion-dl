...
public class ThreadDownloadHTM extends Thread {
	...
	protected JButton Ouvrir, GO, Explorer, telechAll;
	...
	public ThreadDownloadHTM(int nbDeThread, JProgressBar progress,
			...
			JLabel label7, JLabel label8, JButton telechAll)
	...
	public void run() {
		...
		if (nbFichierADL == 0) {
			JOptionPane.showMessageDialog(null, "Aucun lien selectionné",
		...
		for (int i = 0; i <= nbFichierADL; i++) {
			...
			if (i >= 1) {
				if (!EnAttente.isSelectedIndex(i - 1)) {// si la ligne
					// n'est pas
					// selectionnée,
					// on la
					// supprime de
					// la liste
					// d'attente
					ListeAttente.removeElementAt(i - 1);
				...
				if (!EnAttente.isSelectedIndex(i)) {// si la ligne n'est
					// pas selectionnée,
					// on la supprime de
					// la liste
					// d'attente
					ListeAttente.removeElementAt(i);
					nbFichierADL = EnAttente.getModel().getSize();
					LabelNombreaDL.setText("nb de fichier à DL : "
							+ nbFichierADL);
					i = 0;
			...
			if (EnAttente.isSelectedIndex(0)) {// si la ligne est
				// séléctionnée, on dl
				// le fichier
				...
				try {
					...
					Historique.ecrire("Erreur rafraichissement " + e);
				...
				try {
					...
					// on recupere les info du fichier en cours de
					// telechargement
					...
					if (fileLength == -1) {
						...
						// l'url est invalide, on la sort de la liste des ficher
						// a dl mains on ne la met pas dans celle des fichiers
						// deja DL.
						ListeAttente.removeElementAt(EnAttente
								.getSelectedIndex());
						nbFichierADL = EnAttente.getModel().getSize();
						i = 0;
						LabelNombreaDL.setText("nb de fichier à DL : "
					...
					if (ArchiveOrExe == true) {
						...
						while ((read = input.read(buffer)) > 0) {
							...
							TexteVitesse.setText("Vitesse Actuelle : "
									+ Vitesse + " Ko/s");
						...
						// on crée un nombre de fichier correspondant au nombr
						// de thread de dl pour determiner quand le dl est fini
						...
						proc.processRequest(new URL(adresse), RepParDefault,
								p1, p2, p3, p4, p5, p6, p7, p8, label1, label2,
						...
						while (file1exist == true || file2exist == true
							...
							int ProgressTotal = (p1.getValue() + p2.getValue()
									...
									+ p5.getValue() + p6.getValue()
									+ p7.getValue() + p8.getValue())
									/ nbDeThread;
